...
vi.mock("@/lib/api", () => ({
  ...
  getAllCurrentRepresentatives: vi.fn().mockResolvedValue([{ id: 8, name: "First", state: "NY" }, { id: 9, name: "Second", state: "NY" }]),
  compareRepresentatives: vi.fn().mockResolvedValue({ left_representative_id: 8, right_representative_id: 9, congress: 119, shared_vote_count: 2, agree_count: 1, disagreement_count: 1, excluded_shared_vote_count: 1, agreement_rate: 0.5, coverage_complete: false, reason: null, returned_shared_vote_count: 1, shared_votes_truncated: true, shared_votes: [{ vote_id: 7, bill_id: 44, vote_date: "2026-01-01T00:00:00Z", question: "On passage", result: "Passed", left_position: "yes", right_position: "no" }] }),
}));
...
describe("RepresentativeComparePage", () => {
  ...
  it("renders a linkable two-member comparison with shared-vote evidence", async () => {
    ...
    expect(screen.getByText(/Showing 1 of 2 shared vote records/)).toBeVisible();
