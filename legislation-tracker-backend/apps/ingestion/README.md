...
- **`poll_congress`** asks for recently updated bills (House and Senate types), **paginates** the Congress bill list in pages of 250 (`offset`), updates **`IngestionState`** (last poll time, last “update seen” date), and queues **one task per bill** seen across all pages.
- **`backfill_process_bill_versions_for_all_bills`** (optional `session=`): enqueues **`process_bill_versions`** for **every `Bill` row** in the database so you can download text for all ingested bills without waiting on the poll batch.
...
- **`process_bill_versions`** loads text-version metadata and creates **`BillDocument`** rows; **`download_document`** fetches the file from **`source_url`**, uploads to **MinIO** (S3-compatible, free locally) or **`local_media/`** (filesystem), sets **`content_hash`** and optional **`extracted_text`**, then enqueues **`generate_contract`** in `legislation.tasks`.
...
| `tasks.py` | Celery task definitions (`poll_congress`, `process_bill`, `download_document`, …). |
| `document_download.py` | HTTP download, hashing, PDF/XML text extraction, S3/MinIO bucket ensure, upload via Django **STORAGES**. |

## Who should read this

Anyone changing **polling**, **Congress API** behavior, **Celery** configuration, or **retry/dead-letter** handling.

**Document file storage (MinIO/S3/disk):** see **[docs/FILE_STORAGE.md](../../docs/FILE_STORAGE.md)**.
