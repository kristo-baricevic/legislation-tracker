...
- **`process_bill_versions`** loads text-version metadata and creates **`BillDocument`** rows; **`download_document`** fetches the file from **`source_url`**, uploads to **MinIO** (S3-compatible, free locally) or **`local_media/`** (filesystem), sets **`content_hash`** and optional **`extracted_text`**, then enqueues **`generate_contract`** (Phase 5 stub in `legislation.tasks`).
