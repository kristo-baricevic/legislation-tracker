...
| **contract_json** | A structured description (title, summary excerpt, etc.) stored as **JSON** in **PostgreSQL**. |
| **contract_hash** | A **SHA-256** fingerprint of the JSON after we **normalize** it (same content → same hash). |
| **EvidenceSpan** | For each top-level field in the JSON, we store a **short quote** and a **field path** so we can show “where this came from” later. |
...
3. We build a **small JSON contract** from:
   - the bill **title**,
   - a **short excerpt** of `extracted_text` (or title/summary if text is missing),
...
6. Otherwise we **create a new `BillContract`**, update **`Bill.latest_contract`**, set **`BillDocument.contract_generated_at`**, write **`ChangeLog`**, create **`EvidenceSpan`** rows (one per top-level JSON key), then enqueue **Phase 6 stubs**: **`update_topics`**, **`schedule_similarity_for_bill`**.
...
| `apps/legislation/tasks.py` | `generate_contract`, `update_topics` (stub), `schedule_similarity_for_bill` (stub). |
